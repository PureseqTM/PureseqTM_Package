...
usage()
{
	echo "PureseqTM v0.20 [May-03-2019] "
	echo "    Predict TransMembrane (TM) topology from a given sequence in FASTA format. "
	echo ""
	echo "USAGE:  ./PureseqTM.sh <-i input_fasta | input_tgt> [-o out_root] [-m mode] [-p signal_pep] [-P plot] [-K remove_tmp] "
	echo "                       [-l truth_label] [-s high_thres] [-S low_thres] [-c high_len] [-C low_len] [-L seg_len] [-H home] "
	echo "Options:"
	echo ""
	...
	echo ""
	echo "-m mode         : Fast or slow mode. [default = 1 use slow mode] "
	echo "                  set 0 to use fast mode that DO NOT run phobius and philius "
	echo ""
...
#-> others
out_root=""
mode=1              #-> default: RUN with slow mode (run Phobius and Philius)
...
#-> parse arguments
while getopts ":i:o:m:p:P:K:H:l:s:S:c:C:L:" opt;
do
	case $opt in
	#-> required arguments
	i)
...
#-> run Philius to generate DBN features
if [ $mode -eq 1 ]
then
...
detect_model_fast=$param/detect.model_fast
predict_model_fast=$param/puretm.model_fast
detect_model_slow=$param/detect.model_slow
predict_model_slow=$param/puretm.model_slow
profile_model=$param/proftm.model
addi=""

#---- use fast or slow model --------#
if [ $mode -eq 1 ]
then
	featdim=$featdim_slow
	detect_model=$detect_model_slow
	predict_model=$predict_model_slow
else
	featdim=$featdim_fast
	detect_model=$detect_model_fast
	predict_model=$predict_model_fast
fi

#---- discriminate TP and nonTP -----#
